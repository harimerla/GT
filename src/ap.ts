...
pass.dispatchWorkgroups(zeroimg.length/5);
...
while(true){
  ...
  renderPass.setBindGroup(0,uniformBindGroup);
      ...
      // var image = pixelsToCanvas(result, width, height);
      // console.log(image.textContent);
      // const float32Array = new Float32Array(width * height);

// Populate the float32Array with data...
// console.log('Length of result'+result.length);
// const imageData = new ImageData(Uint8ClampedArray.from(result), width, height);
...
// });
      ...
      //   {resultBuff},
      //   //{device.createImageCopyBuffer({resultBuff,bytesPerRow: 256 * 4, rowsPerImage: 256}) },
      //   {cubeTexture },
      ...
      for(var i=0;i<result.length;i+=4){
        ...
        // if(result[i]>150 && result[i]<200){
        //   result[i]=0;
        //   result[i+1]=0;
        //   result[i+2]=255;
        // }
        if(result[i]>90 && result[i]<150){
          result[i+1]=0;
          result[i+2]=128;
        }
        // if(result[i]<128){
